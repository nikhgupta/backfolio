...
class RebalanceOnScoreNseStrategy(BaseStrategy):
    """
    ...
    def __init__(self, rebalance=1, assets=3, max_assets=13,
                 max_order_size=0, min_order_size=1000,
                 max_position_held=0, markup_sell=1, markdn_buy=1,
                 reserved_cash=5, weighted=False, debug=True):
        """
    ...
    def exit_price(self, asset, _symbol, data):
        if self.portfolio.positions[-1][asset] > 0:
            return self.selling_price(_symbol, data)
        else:
            return self.buying_price(_symbol, data)
    ...
    def advice_investments_at_tick(self, _tick_event):
        ...
        for asset, asset_equity in equity.items():
            ...
            if (symbol in data.index and
                ...
                price = self.exit_price(asset, symbol, asset_data)
