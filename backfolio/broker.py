...
class SimulatedBroker(AbstractBroker):
    ...
    def create_order_after_placement(self,
        ...
        order = Order(advice, exchange, quantity, cost, price, comm,
                      comm_asset, comm_cost)
    ...
    def execute_order_after_creation(self, order_created_event, exchange=None):
        ...
        base_rate = 1 if order.base == pbase else self.get_asset_rate(
            order.base)
        ...
        if (np.isnan(symbol_data['close']) and order.is_open
                and not self.datacenter._current_real.empty):
            order.mark_rejected(self,
                                'Symbol closing price missing: %s' % symbol)

        elif (not symbol_data and order.is_open
              and not self.datacenter._current_real.empty):
        ...
        #     # self.datacenter._continue_backtest = False
            ...
            order.mark_rejected(
                self, 'Insufficient Base Asset: %0.8f (cost) vs %0.8f (cash)' %
                (cost, cash))
        elif (order.is_sell and comp8(abs(order.quantity), asset_pos) > 0
              and not pending):
            order.mark_rejected(
                self,
                'Insufficient Tx Asset: %0.8f (quantity) vs %0.8f (available)'
                % (abs(order.quantity), asset_pos))
        elif ((comp8(comm, comm_asset_balance) > 0 or comm_asset_balance < 0)
              and not pending):
            order.mark_rejected(
                self,
                'Insufficient Brokerage: %0.8f (comm) vs %0.8f (asset bal)' %
                (comm, comm_asset_balance))
        elif (order.commission_asset == self.context.base_currency
              and comp8(comm, self.account.cash) > 0):
            order.mark_rejected(
                self, 'Insufficient Cash: %0.8f (comm) vs %0.8f (asset bal)' %
    ...
    def cancel_pending_orders(self, symbol=None):
        for order in self.portfolio.open_orders:
            if not order.is_cancelled and (not symbol
    ...
    def calculate_order_shares_and_cost(self, advice):
        ...
        base_rate = 1 if advice.base == pbase else self.get_asset_rate(
            advice.base)
        ...
        if self.max_position_held >= 1e-8:
            asset_rate = 1 if advice.asset == pbase else self.get_asset_rate(
                advice.asset)
            equity = (advice.position + quantity) * asset_rate
            if abs(equity) > self.max_position_held:
                quantity = (1 if cost > 0 else
                            -1) * self.max_position_held / asset_rate
        ...
        if advice.max_order_size and abs(
                cost) > advice.max_order_size / base_rate:
        ...
        return (round(quantity, 8), round(price * quantity,
                                          8), round(price, 8))
    ...
    def calculate_commission(self, advice, asset_quantity, order_cost):
        ...
        if callable(self.context.commission):
            ...
            comm_cost = self.context.commission / 100. * abs(
                order_cost_in_base)
...
class CcxtExchangePaperBroker(SimulatedBroker):
    ...
    def create_order_after_placement(self,
        ...
        super().create_order_after_placement(order_requested_event,
                                             self.exchange.name)
    ...
    def execute_order_after_creation(self, order_created_event, exchange=None):
        ...
        super().execute_order_after_creation(order_created_event,
                                             self.exchange.name)
...
class CcxtExchangeBroker(CcxtExchangePaperBroker):
    ...
    def cancel_pending_orders(self, symbol=None, refresh=False):
        ...
        if refresh:
            ...
            orders = [
                CcxtLiveOrder(o) for o in self.exchange.fetch_open_orders()
            ]
        ...
        for idx, order in enumerate(orders):
            ...
            try:
                ...
                self.context.notify(
                    ...
                    (order.side, order.id, order.asset, order.fill_price),
                    formatted=True,
                    publish=False)
    ...
    def reject_order(self, order, reason, track=True):
        order.mark_rejected(self, reason, track)
        return order
    ...
    def __place_order_on_exchange(self, order, symbol, quantity, price):
        ...
        try:
            if order.order_type == 'LIMIT' and quantity > 0:
                ...
                resp = self.exchange.create_market_buy_order(symbol, quantity)
            elif order.order_type == 'MARKET' and quantity < 0:
    ...
    def execute_order_after_creation(self, order_created_event, exchange=None):
        ...
        if symbol not in market_data:
            order.mark_rejected(self, 'SYMOBSOLETE: %s' % symbol)
        limits = market_data[symbol]['limits']

        if not quantity:
            return self.reject_order(order, 'QUANTZERO')
        if abs(quantity) < limits['amount']['min']:
            return self.reject_order(order, 'QUANTMIN')
        if abs(cost) < limits['cost']['min']:
            return self.reject_order(order, 'COSTMIN')
        if price < limits['price']['min']:
            return self.reject_order(order, 'PRICEMIN')
        if abs(cost) < self.min_order_size:
            return self.reject_order(order, 'COSTMINSIZE')
